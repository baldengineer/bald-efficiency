...
def main():
	...
	#voltages = np.arange(6,14, 2).tolist()
	voltages = [6]
	#voltages = [6.0, 8.0, 12.0]

	set_points = np.linspace(0.1, 1, 10)
	#set_points = np.logspace(0, 1, endpoint=True) / 10
	#set_points = ["0.111A"]
	#set_points = [0.1, 0.5, 1]
	...
	print("Curr (A),Eff (%),Pd (W),Vout (V)")
	for point in set_points:
		clean_point = f"{float(point):.4}"
		...
		for volt in voltages:
			...
			if ((float(out_measures['power']) == 0) or (float(in_measures['power'])) == 0):
				...
				out_voltage = float(out_measures['voltage'])
				power_dissipated = float(in_measures['power']) - float(out_measures['power'])
				print(f",{float(eff):.2f},{float(power_dissipated):.2f},{float(out_voltage):.2f}", end="", flush=True)
